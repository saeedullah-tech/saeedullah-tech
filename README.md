<div align="center">

# <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&weight=700&size=32&pause=1000&color=00C2FF&center=true&vCenter=true&width=600&lines=Saeed+Ullah;Junior+Data+Analyst;Turning+Data+Into+Insights" />

<img src="https://img.shields.io/badge/📍_Riyadh,_Saudi_Arabia-0A66C2?style=for-the-badge" />
<img src="https://img.shields.io/badge/🎓_B.Sc._Computer_Science-22C55E?style=for-the-badge" />
<img src="https://img.shields.io/badge/💼_Open_to_Work-FF6B35?style=for-the-badge" />

### ✨ *Turning raw data into stories worth telling.*

</div>

---

# 🚀 About Me

```yaml
name: Saeed Ullah
role: Junior Data Analyst
location: Riyadh, Saudi Arabia
education: B.Sc. Computer Science
languages:
  - Urdu (Native)
  - English (Advanced)
  - Arabic (Intermediate)
open_to:
  - Data Analyst
  - Reporting Analyst
  - MIS
  - Operations Support
```

💡 Detail-oriented analyst passionate about transforming messy datasets into actionable insights.

📊 I build dashboards, automate reports, and create visualizations that help decision-makers understand data faster.

---

# 🧠 Skills & Expertise

<table>
<tr>
<td valign="top" width="50%">

### 📈 Data Analytics

* KPI Tracking & Reporting
* Data Cleaning & Validation
* Dashboard Development
* Trend & Performance Analysis
* Business Insights
* Data Visualization

</td>
<td valign="top" width="50%">

### ⚡ Professional Skills

* Problem Solving
* Attention to Detail
* Time Management
* Team Collaboration
* Communication Skills
* Fast Learner

</td>
</tr>
</table>

---

# 🛠️ Tech Stack

<div align="center">

### 📊 Analytics Tools

<img src="https://skillicons.dev/icons?i=mysql" />
<img src="https://img.shields.io/badge/Power_BI-F2C811?style=for-the-badge&logo=powerbi&logoColor=black" />
<img src="https://img.shields.io/badge/Excel-217346?style=for-the-badge&logo=microsoft-excel&logoColor=white" />
<img src="https://img.shields.io/badge/Tableau-E97627?style=for-the-badge&logo=tableau&logoColor=white" />
<img src="https://img.shields.io/badge/Google_Sheets-34A853?style=for-the-badge&logo=googlesheets&logoColor=white" />

### 🌐 Development

<img src="https://skillicons.dev/icons?i=html,css,js,wordpress" />

### ⚙️ Tools

<img src="https://skillicons.dev/icons?i=github,git,vscode" />

</div>

---

# 📂 Featured Projects

<table>
<tr>
<td width="50%">

## 📌 Sales Performance Dashboard

📊 Interactive dashboard built in Power BI to monitor sales KPIs, compare regions, and analyze trends.

**🔧 Tools Used:**
`Power BI` `Excel` `DAX`

**✨ Highlights:**

* Visualized 10K+ rows of sales data
* Reduced manual reporting effort
* Added drill-through filters and KPI cards

</td>
<td width="50%">

## 📌 Customer Data Analysis

📈 SQL-driven project to analyze customer behavior and identify churn factors.

**🔧 Tools Used:**
`MySQL` `Tableau` `Google Sheets`

**✨ Highlights:**

* Identified top 3 churn drivers
* Built visual reports for stakeholders
* Improved understanding of customer trends

</td>
</tr>
</table>

<br>

## 📌 Data Cleaning & Validation Dashboard

🧹 Excel dashboard created to detect missing data, improve accuracy, and track issues.

**🔧 Tools Used:**
`Excel` `Pivot Tables` `Charts`

**✨ Highlights:**

* Flagged missing data automatically
* Created KPI summaries and issue tracking
* Improved reporting accuracy

---

# 📜 Certifications

🏅 Microsoft Excel for Data Analysis
🏅 Power BI Fundamentals
🏅 SQL / MySQL for Data Analysis
🏅 Data Analytics & Business Intelligence
🏅 Front-End Web Development
🏅 WordPress Development

---

# 🌐 Connect With Me

<div align="center">

<a href="mailto:Saeedtech990@gmail.com">
  <img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" />
</a>
<a href="https://github.com/SAEEDULLAH">
  <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" />
</a>
<a href="https://linkedin.com/in/your-link">
  <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
</a>

</div>

---

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=SAEEDULLAH&show_icons=true&theme=tokyonight&hide_border=true" height="170" />
<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=SAEEDULLAH&layout=compact&theme=tokyonight&hide_border=true" height="170" />

<br><br>

### ⭐ "Data is the new oil — but only if you know how to refine it."

</div>
